...
export default {
    ...
    setup(){
        ...
        let list=ref([])//收货地址列表
        let cartList=ref([])//购物车中需要结算的商品列表
        let cartTotalPrice=ref(0)//商品总金额
        let count=ref(0)//商品结算数量
        let checkedItem=ref({})//选中的商品对象
        let userAction= ref('')//用户行为 0：新增 1：编辑 2：删除
        let showDelModal=ref(false)//是否显示删除弹框
        let showEditModal=ref(false)//是否显示新增或者编辑弹框
        let checkIndex=ref(0)//当前收货地址选中索引
        let getAddressList=()=>{
            axios.get('/shippings').then((res) => {
                list.value = res.list;
            })
        }
        let openAddressModal=()=>{
            userAction.value = 0;
            checkedItem.value = {};
            showEditModal.value = true;
        }
        let editAddressModal=(item)=>{
            userAction.value = 1;
        ...
        let submitAddress=()=>{
            ...
            if (userAction.value == 0) {
                ...
                method = 'put', url = `/shippings/${checkedItem.value.id}`;
            } else {
                method = 'delete', url = `/shippings/${checkedItem.value.id}`;
            }
        ...
        let getCartList=()=>{
            axios.get('/carts').then((res) => {
                let l = res.cartProductVoList;//获取购物车中所有商品数据
                cartTotalPrice.value = res.cartTotalPrice;//商品总金额
                cartList.value = l.filter(item => item.productSelected);
     ...
     .edit-wrap{
      ...
      .item{
        ...
        .input{
          ...
          padding-left:15px;
          border:1px solid #E5E5E5;
          // +  选择相邻元素，即后一个元素紧跟在前一个之后，并且共享同一个父节点;选择input标签后紧挨的iput标签
          &+.input{
            margin-left:14px;
          }
        }
